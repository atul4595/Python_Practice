...
class check(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        url = 'https://www.amazon.in/s?' + urlencode(payload)
    ...
    def parse(self, response):
        ...
        urls = response.xpath("//div[@class='a-section a-spacing-none']//a[@class='a-link-normal a-text-normal']//@href").extract()
        for ur in urls:
            # ur = response.urljoin(ur)
            ur='https://www.amazon.in'+ ur
            ...
            # time.sleep(3)
            # yield scrapy.Request(ur, callback=self.parse_details,headers=self.headers)
            # yield response.follow(ur,callback=self.parse_details)
            ur=''
